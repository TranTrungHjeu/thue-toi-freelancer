...
# (Tuỳ chọn) Gemini - chỉ cần nếu bạn dùng tính năng phân tích CV (PDF)
GEMINI_API_KEY=your_gemini_api_key
# GEMINI_MODEL=... (tuỳ chọn; nếu bỏ trống backend sẽ tự chọn model hợp lệ)
```
...
Nếu bạn vừa thêm/sửa biến trong `.env`, hãy chạy lại `docker compose up -d --build` (hoặc `docker compose restart backend`) để container backend nhận biến mới.
