...
- Tên bảng: `users`, `service_categories`, `service_gigs`, `orders`, `order_details`.
- Tên cột: `id`, `first_name`, `email`, `created_at`.
- Khoá ngoại (Foreign Key): `<table>_id`. VD: `user_id`, `category_id`.
...
| `POST` | Hành động logic (Phi chuẩn) | `/api/v1/orders/1/checkout` | `/api/v1/checkoutOrder/1` |
